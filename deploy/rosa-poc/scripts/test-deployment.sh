...
print_test "Sending basic query (may take 30-60s)..."
query_response=$(curl -sk --max-time 120 -X POST "https://${LIGHTSPEED_URL}/v1/query" \
...
print_test "Sending RAG query about RHOAI (may take 30-60s)..."
rag_query='What are the key features of Red Hat OpenShift AI?'

rag_response=$(curl -sk --max-time 120 -X POST "https://${LIGHTSPEED_URL}/v1/query" \
    -H "Content-Type: application/json" \
...
stream_response=$(curl -sk --max-time 120 -X POST "https://${LIGHTSPEED_URL}/v1/streaming-query" \
    -H "Content-Type: application/json" \
